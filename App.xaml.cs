            ...
            if (CheckClientIsOpen())
            {
                try
                {
                    ...
                    if (gamephase == "\"ChampSelect\"")
                    {
                        Action message = () =>
                        {
                            if (!autoGenerated && window.LoadingText.Text != "Waiting For A Champion To Be Picked" && window.LoadingText.Text != "Loading Champion Interface" && window.LoadingText.Text != "Error, No Champion Data Exists")
                            {
                        ...
                        if (championHoverId != 0)
                        {
                            ...
                            if (!championJson.Contains("404 Not Found"))
                            {
                                ...
                                if (champion == default || (championJsonObject.name != "Nunu & Willump" && champion != regex.Replace(championJsonObject.name, "")) || (championJsonObject.name == "Nunu & Willump" && champion != "nunu"))
                                {
                                    ...
                                    if (htmlDoc.Text.Contains("Recommended Item Builds"))
                                    {
                                        Action act = () =>
                                        {
                                    ...
                                    else
                                    {
                                        Action act = () =>
                                        {
                                            ...
                                            window.LoadingText.Text = "Error, No Champion Data Exists";
                                            window.LoadingIcon.Visibility = Visibility.Visible;
                                        };
                                        window.Dispatcher.Invoke(act);
                        ...
                        if (LoL_Generator.Properties.Settings.Default.EnableCheckBox && generatedFor != champion + currole && championLockId != "0" && !string.IsNullOrEmpty(championLockId) && int.TryParse(championLockId, out int i) && champion != default && currole != default)
                        {
                            autoGenerated = true;
