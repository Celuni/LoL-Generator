...
namespace LoL_Generator
{
    ...
    public partial class App : Application
    {
                ...
                try
                {
                    ...
                    if (gamephase == "\"ChampSelect\"")
                    {
                        ...
                        if (LoL_Generator.Properties.Settings.Default.EnableCheckBox && !autoGenerated && generatedFor != champion + currole && championLockId != "0" && !string.IsNullOrEmpty(championLockId) && int.TryParse(championLockId, out int i) && champion != default && currole != default)
                        {
                            ...
                            Console.WriteLine($"EnableCheckBox: {LoL_Generator.Properties.Settings.Default.EnableCheckBox}");
                            Console.WriteLine($"autoGenerated: {autoGenerated}");
                            Console.WriteLine($"generatedFor:  {generatedFor}");
                            Console.WriteLine($"championLockId: {championLockId}");
                            Console.WriteLine($"championLockId != 0: {championLockId != "0"}");
                            Console.WriteLine($"int.TryParse(championLockId, out int i): {int.TryParse(championLockId, out int j)}");
                            Console.WriteLine($"!string.IsNullOrEmpty(championLockId): {!string.IsNullOrEmpty(championLockId)}");
                            Console.WriteLine($"champion: {champion}");
                            Console.WriteLine($"currole: {currole}");
        ...
        async void GenerateRunePage(string champion, string role)
        {
            ...
            {
                window.Dispatcher.Invoke(new Action(() => window.LoadoutProgress.Value += 5));
            }
        }
